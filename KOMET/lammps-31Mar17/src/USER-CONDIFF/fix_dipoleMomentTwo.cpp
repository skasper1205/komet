...
FixDipoleMomentTwo::FixDipoleMomentTwo(LAMMPS *lmp, int narg, char **arg) :
		Fix(lmp, narg, arg)
{
	...
	file = fopen("dipole_moment_two.cor","w");
	fclose(file);
	file = fopen("dipole_moment_two.cor","a");

}

//The class destructor
FixDipoleMomentTwo::~FixDipoleMomentTwo()
{
   fclose(file);
}

//Where algorithm steps in
int FixDipoleMomentTwo::setmask()
{
	int mask = 0;
	mask |= END_OF_STEP;
	return mask;
...
void FixDipoleMomentTwo::end_of_step()
{
...
void FixDipoleMomentTwo::calc_cm()
{
...
void FixDipoleMomentTwo::calc_dipole_moment()
{
	...
	while(R<6){
	  ...
	  for (int i = 0; i < nlocal; i++) {
	    ...
	    if(mask[i] & groupbit_coions || mask[i] & groupbit_counterions){
	      ...
	      if(R <= x_help && x_help < R+r && sqrt(pow(y_help,2)+pow(z_help,2)) <
	      r_tube){
			p += q[i]*x_help;
	      }
	    }
