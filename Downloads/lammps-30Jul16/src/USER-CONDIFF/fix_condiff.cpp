		...
		Fix(lmp, narg, arg)
{
	if (narg < 7) error->all(FLERR, "Illegal fix condiff command");
	  ...
	  accuracy_relative = force->kspace->accuracy_relative;
	  ...
	  memory->create(acons,8,7,"condiff:acons");
	  acons[1][0] = 2.0 / 3.0;
	  acons[2][0] = 1.0 / 50.0;
	  acons[2][1] = 5.0 / 294.0;
	  acons[3][0] = 1.0 / 588.0;
...
void FixCondiff::init_condiff()
{
  ...
  accuracy=force->kspace->accuracy;
  accuracy_absolute=force->kspace->accuracy_absolute;
  accuracy_relative=force->kspace->accuracy_relative;

  two_charge_force =  force->kspace->two_charge_force;
  ...
  stagger_flag = force->kspace->stagger_flag;
  differentiation_flag = force->kspace->differentiation_flag;
...
void FixCondiff::setup_triclinic()
{
  int i,j,k,n;
  double *prd;
  ...
  for (k = nzlo_fft; k <= nzhi_fft; k++) {
    ...
    for (j = nylo_fft; j <= nyhi_fft; j++) {
      ...
      for (i = nxlo_fft; i <= nxhi_fft; i++) {
        ...
        force->kspace->x2lamdaT(&unitk_lamda[0],&unitk_lamda[0]);
        fkx[n] = unitk_lamda[0];
        fky[n] = unitk_lamda[1];
        fkz[n] = unitk_lamda[2];
        n++;
...
void FixCondiff::compute()
{
  ...
  if (force->kspace->slabflag == 1) slabcorr();
...
void FixCondiff::allocate_peratom()
{
...
void FixCondiff::deallocate_peratom()
{
  peratom_allocate_flag = 0;
...
void FixCondiff::set_grid_global()
{
  ...
  if (!gridflag) {
    ...
    if (triclinic) {
      ...
      tmp[1] = ny_pppm/yprd;
      tmp[2] = nz_pppm/zprd;
      force->kspace->lamda2xT(&tmp[0],&tmp[0]);
  ...
  if (triclinic == 0) {
    ...
    tmp[1] = ny_pppm;
    tmp[2] = nz_pppm;
    force->kspace->x2lamdaT(&tmp[0],&tmp[0]);
    h_x = 1.0/tmp[0];
    h_y = 1.0/tmp[1];
    h_z = 1.0/tmp[2];
  }
...
int FixCondiff::factorable(int n)
{
  int i;
...
double FixCondiff::compute_df_kspace()
{
  double xprd = domain->xprd;
  double yprd = domain->yprd;
  double zprd = domain->zprd;
...
double FixCondiff::compute_qopt()
{
  double qopt = 0.0;
...
double FixCondiff::estimate_ik_error(double h, double prd, bigint natoms)
{
  double sum = 0.0;
  for (int m = 0; m < order; m++)
    sum += acons[order][m] * pow(h*g_ewald,2.0*m);
  double value = 0;//q2 * pow(h*g_ewald,(double)order) *
...
double FixCondiff::newton_raphson_f()
{
  double xprd = domain->xprd;
  double yprd = domain->yprd;
  double zprd = domain->zprd;
  ...
  double df_rspace = 1;//2.0*q2*exp(-g_ewald*g_ewald*cutoff*cutoff) /
...
double FixCondiff::final_accuracy()
{
  double xprd = domain->xprd;
  double yprd = domain->yprd;
  double zprd = domain->zprd;
  ...
  double df_kspace = compute_df_kspace();
  double q2_over_sqrt = 1;//q2 / sqrt(natoms*cutoff*xprd*yprd*zprd);
  double df_rspace = 2.0 * q2_over_sqrt * exp(-g_ewald*g_ewald*cutoff*cutoff);
  double df_table = 1;//estimate_table_accuracy(q2_over_sqrt,df_rspace);
...
void FixCondiff::set_grid_local()
{
  ...
  else force->kspace->kspacebbox(cuthalf,&dist[0]);
...
void FixCondiff::compute_gf_denom()
{
  int k,l,m;
...
void FixCondiff::compute_gf_ik_triclinic()
{
  double snx,sny,snz;
  double argx,argy,argz,wx,wy,wz,sx,sy,sz,qx,qy,qz;
  double sum1,dot1,dot2;
...
void FixCondiff::compute_sf_precoeff()
{
  int i,k,l,m,n;
...
void FixCondiff::poisson_ik_triclinic()
{
  int i,j,k,n;
...
void FixCondiff::poisson_peratom()
{
  int i,j,k,n;
...
void FixCondiff::fieldforce_ik()
{
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    f[i][0] += qfactor*ekx;
    f[i][1] += qfactor*eky;
    if (force->kspace->slabflag != 2) f[i][2] += qfactor*ekz;
  }
}
...
void FixCondiff::fieldforce_ad()
{
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    sf *= 2*q[i]*q[i];
    if (force->kspace->slabflag != 2) f[i][2] += qfactor*(ekz*q[i] - sf);
  }
}

void FixCondiff::fieldforce_peratom()
{
  int i,l,m,n,nx,ny,nz,mx,my,mz;
  FFT_SCALAR dx,dy,dz,x0,y0,z0;
...
void FixCondiff::pack_forward(int flag, FFT_SCALAR *buf, int nlist, int *list)
{
  int n = 0;
...
void FixCondiff::unpack_forward(int flag, FFT_SCALAR *buf, int nlist, int *list)
{
  /*int n = 0;
...
void FixCondiff::pack_reverse(int flag, FFT_SCALAR *buf, int nlist, int *list)
{
  if (flag == REVERSE_RHO) {
...
void FixCondiff::unpack_reverse(int flag, FFT_SCALAR *buf, int nlist, int *list)
{
  if (flag == REVERSE_RHO) {
...
void FixCondiff::procs2grid2d(int nprocs, int nx, int ny, int *px, int *py)
{
...
int FixCondiff::timing_1d(int n, double &time1d)
{
  double time1,time2;
...
int FixCondiff::timing_3d(int n, double &time3d)
{
  double time1,time2;
...
double FixCondiff::memory_usage()
{
...
void FixCondiff::compute_group_group(int groupbit_A, int groupbit_B, int AA_flag)
{
...
void FixCondiff::allocate_groups()
{
...
void FixCondiff::deallocate_groups()
{
  group_allocate_flag = 0;
...
void FixCondiff::make_rho_groups(int groupbit_A, int groupbit_B, int AA_flag)
{
  int l,m,n,nx,ny,nz,mx,my,mz;
  FFT_SCALAR dx,dy,dz,x0,y0,z0;
...
void FixCondiff::poisson_groups(int AA_flag)
{
  int i,j,k,n;
...
void FixCondiff::poisson_groups_triclinic()
{
  int i,n;
...
void FixCondiff::slabcorr_groups(int groupbit_A, int groupbit_B, int AA_flag)
{
